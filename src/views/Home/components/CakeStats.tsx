...
const CakeStats = () => {
  ...
  const bigfootPrice = usePriceCakeBusd()
  const circSupply = totalSupply ? totalSupply.minus(burnedBalance) : new BigNumber(0)
  const cakeSupply = getBalanceNumber(circSupply)
  const marketCap = bigfootPrice.times(circSupply)

  let bigfootPerBlock = 0
  if (farms && farms[0] && farms[0].bigfootPerBlock) {
    bigfootPerBlock = new BigNumber(farms[0].bigfootPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="lg" mb="24px">
          {TranslateString(534, 'FOOT Stats')}
        </Heading>
        <RowHighlighted>
          <Text fontSize="14px">
            <strong>
        ...
        <Row>
          <Text fontSize="14px">
            {TranslateString(538, 'FOOT eaten by BigFoot')}{' '}
        ...
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New FOOT/block')}</Text>
